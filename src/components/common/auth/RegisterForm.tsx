...
export function RegisterForm() {
    ...
    return (
        ...
        <form onSubmit={handleSubmit(submitHandle)} className="shadow-Xl p-12 rounded-2xl w-full max-w-xl lg:max-w-none mx-auto lg:w-[45%] bg-white text-sm lg:text-base">
            ...
            <Input register={register('username', {required: true})} type={'text'} label={'Username'} errors={errors} {...{placeholder: 'Enter Username'}}/>
            <Input register={register('email', {required: true})} type={'email'} label={'Email'} errors={errors} {...{placeholder: 'Enter your Email'}}/>
            ...
            <div className="flex flex-row gap-3">
                ...
                <label htmlFor="termsCheck" className="text-secondary-100">Accept <span className="text-primary-100 font-bold">terms & conditions</span></label>
            </div>
            <div className={'flex text-secondary-100 mb-6 text-sm'}>
                <p>have an account?</p>
                <span className={'hover:text-primary-100 cursor-pointer pl-1 font-medium'} onClick={()=>{setShowModal(true)}}>Login</span>
            </div>
